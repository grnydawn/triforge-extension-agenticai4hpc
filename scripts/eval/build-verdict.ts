...
function classify(oracleOutcome: string, flagged: boolean): string {
  const rejects = oracleOutcome === 'startup-reject' || oracleOutcome === 'ran-but-diverged';
  if (rejects && flagged) return 'agree-fault';
  if (oracleOutcome === 'ran-to-completion' && !flagged) return 'agree-clean';
  if (oracleOutcome === 'ran-to-completion' && flagged) return 'tool-stricter';
  if (rejects && !flagged) return 'tool-missed';
  return 'unclassified';
}
...
function main(): void {
  ...
  const out = rawEntries.map((entry) => {
    ...
    // "Flagged" must mean what detection means in scoreFixture: ANY non-advisory finding,
    // whatever its severity. Testing severity==='error' instead hid five faulty fixtures whose
    // finding is a warning (degenerate forcing, hydrograph coverage, value range, and the two
    // expectation faults) in the agree-clean bucket, contradicting the corpus report.
    const flagged = findings.some((f) => !f.advisory);
    ...
    const agreement = classify(oracleOutcome, flagged);

    // eslint-disable-next-line no-console
    console.log(`${entry.dir}: oracle=${oracleOutcome} flagged=${flagged} -> ${agreement}`);
