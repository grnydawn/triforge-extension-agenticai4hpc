...
out-of-range runoff-indexing bug that silently corrupted a production GPU simulation. We generalize
...
\caption{A real ``works-but-wrong'' failure in an operational TRITON run over the French
Broad River basin, western North Carolina ($4458\times4802$ cells at $20$\,m). (a)~Final
maximum water depth: a physical dendritic flood (shaded, $0$--$2$\,m) \emph{plus} a hard-edged
$1.5\times10^{214}$\,m block in the southeast (hatched). (b)~The runoff zone map: the block
...
this evaluation, so we also ran the tool on a real failure: the operational project behind
...
logarithmic colour scale, dry cells left blank). \textbf{Left:} the uncorrected run, where a runoff
map numbered from $1$ causes an out-of-range table read that injects a constant
$\sim$$1.5\times10^{214}$\,m block in the southeast (hatched), flattening the real flood
network at the bottom of a scale spanning more than $200$ orders of magnitude. \textbf{Right:} the
corrected run, numbered from $0$: realistic, terrain-following depths (maximum $34.0$\,m), and
...
about a second on a laptop. A TRITON forecasting project consumes on the order of $200$
GPU-hours a year, so a deployment that catches even two or three setup errors a quarter saves
...
(4-thread) Intel Core~i5-4250U with 7\,GB RAM. The two operational runs of
