...
flood forecasting; TRITON can run on one or many GPUs. A
...
terrain, such as ground elevation) and time series (runoff hydrographs, which tabulate discharge
...
($4458\times4802$ cells). (a)~Final maximum water depth: a physical dendritic flood
...
count of runoff zones or water sources, a run duration, a rainfall range. Codex
(\texttt{gpt-5.6-sol}) is perfect here with and without the tool. Claude
(\texttt{claude-sonnet-5}) on its own misses two of five; the tool restores it to $5/5$. Once an
...
header exactly as the solver does. We checked the opposite direction too: the agents kept
...
(4-thread) Intel Core~i5-4250U with 7\,GB RAM. The operational run of Figures~\ref{fig:anomaly}
and~\ref{fig:beforeafter} used 40 NVIDIA~A100 GPUs (10 nodes $\times$ 4) on a custom
computing cluster.
