...
importantly, raises how often the agent points at the \emph{right} fault, not merely a real one,
...
already raises none. Third, the tool \emph{reduces} computational cost: \textbf{$\sim$1.3--1.6$\times$ fewer}
...
\emph{real} project, where the tool statically catches, in about a second, an
out-of-range runoff-indexing bug that silently corrupted a real multi-GPU simulation. We generalize
...
\begin{itemize}
  \item \textbf{It works from the project's own files.} It reads the solver's own \texttt{.cfg}
        ...
        actually loads are its single source of truth, determining the physics of the simulation
        before a single grid cell is processed.
...
\begin{enumerate}
  ...
  \item A transparent, end-to-end evaluation, published for inspection. A $\pm$tool ablation
        ...
        lower cost, not better raw detection), and a real TRITON test case validates the tool
...
\caption{A real ``works-but-wrong'' failure in a TRITON forecast test case over the French
...
Figure~\ref{fig:anomaly} comes from a TRITON flood-forecast test case: a $10$-day run over the
French Broad River basin in western North Carolina, produced by a system still under active
development. The solver ran on eight NVIDIA~A100 GPUs, finished in about $28$~minutes,
and wrote a complete set of output maps, reporting success. Yet
...
a read-only tool such as diagnosis runs on its own, while the tools whose effects reach outside the
project (starting a run, or unpacking material into one) first stop and ask the person to approve
...
and boundaries, a landscape with zero friction everywhere, a runoff file with the wrong
...
Our experiment compares two \emph{arms}, which are identical except for one thing, whether the
tool is available:
\begin{itemize}
...
We report the results in two parts, neither of which uses any AI. The first asks how accurate the
tool is on its own, and checks the answer key it is graded against. The second asks the central
...
it with \texttt{TRITON\_EXE}) and watch what the solver does. Section~\ref{sec:realworld} adds a second, independent check: the tool on a
...
the Wilson intervals in Table~\ref{tab:results}, so we treat it as the headline result.
...
but the agent does not always trust it. The ``don't cry wolf'' benefit is real, but it lands on
the weaker client rather than on both.
...
capable agent does reliably, so we treat this group as a boundary of the effect rather than a
headline result.
...
diagnosis text read back) costs only about \textbf{$90$--$110$ tokens}, under $1.5\%$ of
...
class only the tool catches (Section~\ref{sec:oracle}). The $32$-project test set was built for
...
meaningless, a fault that never shows up as a crash. The mistake was not in
...
$\sim$$1.5\times10^{214}$\,m block in the southeast, hatched because plotting it
would compress every physical depth into the bottom of the scale. \textbf{Right:}
...
in the difference. What remains is the judge's plain error rate, which we measured directly. A domain expert re-graded \emph{all} $248$ ablation runs by
...
Two stronger, independent judges confirm that this result is not an artifact
...
and $94\%$ of cells, and rate the \emph{bare} arm no higher. A stronger judge, in other words,
...
labels rest on the output-sanity scan and a human audit of the small residual. AI-generated
fixtures do not weaken this: the labels rest on the solver and the human audit, not on who wrote
the files.
...
make the \emph{recurring} problems reliable, not to catch everything. Adding a check is also
...
parallel-I/O layouts that split one input across many files, are left for future work. The second
should suit a counting check: when one input becomes thousands of files, comparing a declared
count against what is on disk is the only practical test, and it would cost the tool no more than
checking a single file.

\paragraph{How far the results generalize} Our results cover one solver, two AI clients, and one
machine, with three repeats per faulty project and one per clean or user-rule project. Because
some of the groups are small, Table~\ref{tab:results} gives a $95\%$ interval for every rate; we
use Wilson intervals, since the usual formula breaks down at small samples and near $0$ or
$100\%$. The setup-fault columns rest on $45$ runs, and their intervals are narrow enough that
the localization gain ($78$--$80\%\!\to\!98\%$) is not in doubt. The clean and user-rule groups
...
since the same $32$ projects give the same answer every time, so it carries no interval. Nothing in
...
\emph{fully static}: it never runs the solver at all, not even a dry run, and it works on any
hand-built TRITON project, not only projects a particular generation pipeline produced. Notably, MatClaw~\cite{matclaw}
...
actually changes what the agent finds, through a controlled with-versus-without comparison.
...
clients, graded blind against solver-confirmed labels. The nearest work stops
short: \cite{lara} validates inside a generation pipeline using dry runs, the
software-engineering line runs code repositories, and the configuration-checking line tests the
checker, not the agent.

\section{Conclusion}
\label{sec:conclusion}
A small, static tool measurably improves an AI assistant's ability to catch ``works but is
wrong'' HPC faults before any compute is spent. It works from the project's own setup files and
reaches the agent through standard interfaces. The result holds across two independent AI
clients, and the tool \emph{reduces} the agent's turns and cost rather than adding to them. The test-case run
...
through the solver. Second, when a language model grades a language model, the grading needs
auditing as hard as the result. Our labels are settled by the real solver rather than by a model,
a human re-graded all $248$ ablation runs, and two independent judges, one stronger and one from a
different model family, reproduce the localization gap. Such checks earn their cost only if they
can overturn the result: ours did, correcting a project we had mislabeled clean. A human grader
is not the final oracle either, and two of our own re-grades were wrong; only the
solver-confirmed label settled them. Third, these faults live exactly where a convention
looks standard but is not. The upstream pipeline numbered runoff zones from $1$, as geographic information system tools do,
while the solver indexes from $0$; and our tool-free agents repeatedly insisted that the runoff
map needed the elevation file's six-number header, which the solver in fact rejects there.
Both are setups that satisfy surface plausibility while violating the solver's own semantics,
which is precisely what \texttt{diagnose\_project} encodes.
...
client, arm, fixture, trial, gold label, judge verdict, human verdict, and a one-line note; it
tallies the $247/248$ agreement quoted in Section~\ref{sec:threats}, and two notes record why
first-pass verdicts were reversed. \texttt{clean-cell-human-audit.csv} and its phase-2 companion
($32$ and $48$ rows) audit the clean-project cells that carry the don't-cry-wolf claim. The human
pass is a record rather than a script, so re-running it means re-grading: the browsable
artifact~\cite{artifact-human} presents all 248 answers with their gold labels for exactly that. It opens by asking whether to show the
recorded verdicts or to start blank, so a reviewer can repeat the pass unanchored and compare
only afterwards. The multi-judge check needs no re-running: its four graded CSVs and all $496$
per-cell rationales ship under \texttt{runs/robustness/}, so the judge-strength claim of
Section~\ref{sec:threats} recomputes from the shipped files with
\texttt{tabulate-ablation.mjs}. \texttt{judge-multi.sh} regenerates them under Opus and
\texttt{gpt-5.6-sol} for anyone reproducing the grading itself.
