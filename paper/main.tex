...
\begin{itemize}
  ...
  \item \textbf{One implementation, offered wherever the agent lives.} The same core logic is
        ...
        itself is read-only, and the tools that start a run or unpack files from outside the
        project first ask the user for approval.
\end{itemize}
...
marker) at the start of a binary elevation file and skips past it to the data; every \emph{other}
binary raster (runoff map, Manning field, initial depth) instead carries a two-value
\texttt{[nrows,\,ncols]} header that the solver \emph{validates}, aborting on a mismatch. No
binary input is header-less; the two kinds of raster simply carry different headers, read by
different overloads (we confirmed all of this in the TRITON source). Those
...
complaint pushed the intended fault aside on the binary projects. Because only the \emph{bare}
arm was reading the files itself, the flaw cost that arm alone, and so \emph{overstated} the
...
\noindent\textbf{The tool's advantage survives correcting the test data.} After we rebuilt every
binary project with the header the real solver requires (Section~\ref{sec:fidelity}) and re-ran
the experiment, the \emph{+tool} scores were essentially unchanged and a clear advantage over
\emph{bare} remained.
...
each check tied to a specific unchecked read in the TRITON source. Counting rather than running
is also what should let the family scale: a parallel-I/O layout can split one input across
thousands of files, where comparing a declared count against what is on disk is the only practical
check, and it would cost the tool no more than checking one.
...
inspection~\cite{artifact-human}. In all three disagreements, the human gave a \emph{bare}-arm
answer credit the judge had scored as a miss; one of the three the judge had left ungraded. The
judge therefore \emph{under}-rates the bare arm, if anything, and cannot inflate the tool's
advantage.
...
only widens the gap. This concern applies only to the with-versus-without comparison: the tool's own accuracy is
...
parallel-I/O layouts that split one input across many files, are left for future work.
...
checks belong to this family and differ in two ways. What a TRITON configuration points at is not
more settings but large data files, so a check must open them (read a grid's header, count a
runoff table's columns, find the largest zone number in a map), not just read the
configuration. And that work asks whether the checker finds the mistake; we ask whether giving
the checker to an AI agent changes what the \emph{agent} concludes.
...
reaches the agent through standard interfaces. The tool does this while \emph{reducing} the
agent's turns and cost. The result holds across two independent AI clients, and the tool's own
overhead is negligible. The compute it saves is far larger than that overhead. The test-case run
...
not. The upstream pipeline numbered runoff zones from $1$, as geographic information system tools do,
...
all 248 answers with their gold labels for exactly that. It opens by asking whether to show the
