...
export class DemLayer {
    ...
    private mapType: string = 'Terrain';
    ...
    private render(min?: number, max?: number) {
        ...
        const { values, noData, header } = this.data;
        ...
        for (let r = 0; r < rows; r++) {
            for (let c = 0; c < cols; c++) {
                ...
                if (val === noData || val < min! || val > max!) {
                    ...
                    let [R, G, B] = Colors.getColor(norm, this.mapType);

                    if (this.hillshade) {
                        const rUp = r > 0 ? r - 1 : r;
                        const rDn = r < rows - 1 ? r + 1 : r;
                        const cL = c > 0 ? c - 1 : c;
                        const cR = c < cols - 1 ? c + 1 : c;
                        const vU = values[rUp][c] === noData ? val : values[rUp][c];
                        const vD = values[rDn][c] === noData ? val : values[rDn][c];
                        const vL = values[r][cL] === noData ? val : values[r][cL];
                        const vR = values[r][cR] === noData ? val : values[r][cR];
                        const dzdx = ((vR - vL) / (2 * cellsize)) * zFactor;
                        const dzdy = ((vU - vD) / (2 * cellsize)) * zFactor;
                        const Lx = -0.5, Ly = 0.5, Lz = 0.707;
                        const lenN = Math.sqrt(dzdx * dzdx + dzdy * dzdy + 1);
                        const Nx = -dzdx / lenN;
                        const Ny = (vU - vD) / (2 * cellsize) * zFactor / lenN;
                        const Nz = 1 / lenN;
    ...
    private updateBoundary() {
        ...
        this.boundaryLayer = L.polygon(boundaryLatlngs, {
            color: '#333333', weight: 2, dashArray: '5, 5', fill: false, interactive: false
        });
    ...
    public updateTransform() {
        ...
        // Calculate and cache matrix in ProjectionManager
        // Note: MapController might want to coordinate this if multiple layers share the same bounds/transform
        // ensuring ProjectionManager.update is called.
        // For now, DemLayer updates it.
